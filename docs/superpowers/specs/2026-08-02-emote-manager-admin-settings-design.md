...
- The setting links directly to the dynamic `/admin/emotes` plugin router URL.
...
- Unauthorized manager page requests return a clear HTML `403` response stating that administrator access is required.
...
2. The administrator opens the Emote Manager link in a new tab.
3. The browser session authenticates the `GET /admin/emotes` request.
4. The server renders the current emote map.
5. Upload, save, and delete requests use the same browser session and are checked by the same administrator authorization helper.
...
- Confirm an administrator can open the manager, view emotes, upload an image, save a code, and delete an emote.
